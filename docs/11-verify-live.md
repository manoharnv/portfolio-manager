...
| 15 | **Observed 2026-09-25:** `api.dhan.co` is CloudFront — successive lookups returned `108.158.46.x` then `18.161.246.x` — so the IP allowlist filled 10 minutes earlier matched the engine's own lookup only by luck; a miss is a dropped connection and a tick failing closed with `fetch failed`. `egress-allowlist.sh` now resolves via `host` (DNS only) and **pins every allowlisted hostname in `/etc/hosts`** to one address the sets contain, refreshed each cycle; network errors also carry their `cause` (ENETUNREACH/EAI_AGAIN…). Verify: `grep -A8 'BEGIN pm-egress-allowlist' /etc/hosts` on the VM, and no `pm-strategy-egress-drop` lines in `journalctl -k` during ticks. Residual: a pinned CloudFront node retiring inside a 10-minute cycle. | `infra/vm/egress-allowlist.sh` | 🟠 Random tick failures — strategies silently miss evaluations. |
