...
| 9 | **Engine reads its read-creds secret once, at start-up** (`main()` → `loadSecret`). The backend now rewrites `pm-strategy-read-creds` after every login/switch, but a running engine keeps the old token until restarted — so a Dhan login at 08:30 does not reach the engine until pm-strategy restarts. Follow-up: re-read the secret (and rebuild the read adapter) when the session it holds is expired or the secret version changed; until then restart the unit after the morning login (`sudo systemctl restart pm-strategy`). | `index.ts` | 🟠 Engine runs the day on yesterday's token → every tick fails closed. |
...
| 7 | **Dhan daily login completes server-side**: the app opens the consent URL from `/v1/auth/dhan/login-url` in the system auth session and only ever sees the backend's verdict in the `pm://broker-callback?broker=dhan&status=ok\|error` bounce — no token, no callback POST (docs/06 §6.7 holds). Verify on a device that `ASWebAuthenticationSession`/Chrome Custom Tabs return the `pm://` redirect issued by the backend's 302 (a cross-site chain: auth.dhan.co → portfolio.swasthionline.com → pm://). Kite is end-to-end. | `src/lib/brokerLogin.ts` | 🟠 No Dhan trading day can start from the app until verified. |
