...
| 18 | **Confirmed live 2026-09-25** (first real login from the app: consent generated, Dhan redirected to `/v1/auth/dhan/redirect?tokenId=…`, consume returned `accessToken` + `dhanClientId` + `expiryTime` as IST wall-clock → parsed to `2026-09-26T09:54:19+05:30`, i.e. 24 h). Originally: **Consent flow** (`consent.ts`, transcribed from the v2 docs): `POST https://auth.dhan.co/app/generate-consent?client_id=…` → `consentAppId`; browser `GET /login/consentApp-login?consentAppId=…` → redirect `?tokenId=…`; `GET /app/consumeApp-consent?tokenId=…` → `accessToken`, `expiryTime` (assumed IST wall-clock `YYYY-MM-DD HH:mm:ss`), `dhanClientId`. Headers `app_id`/`app_secret` on both calls. Also: the API key form's "Static IP" must hold the VM's IP for order endpoints, and there is a 25 consents/day cap. | `consent.ts` | 🟠 Daily login fails closed (no session) — nothing trades. |
| 19 | **Confirmed live 2026-09-25:** an account with no demat holdings gets **HTTP 500 `DH-1111` "No holdings available"** from `GET /holdings` — an error envelope for an empty result. The adapter maps that (and the same wording for positions/orders) to `[]` via `isDhanEmptyResult`; it failed every strategy tick closed before. Still assumed: positions and the order book use the same code/wording when empty. | `errors.ts`, `adapter.ts` | 🟠 Empty portfolio = every tick fails; nothing is ever proposed. |
...
| 3 | **Resolved live (2026-09-24; retirement confirmed 2026-09-25 — `dhan-access-token` v1 DESTROYED after v2 was added):** the expiry used to be a **label** on the secret, and every `get` failed closed as "not set" — `roles/secretmanager.secretAccessor` is `versions.access` only (no `secrets.get`) and `secretVersionManager` has no `secrets.update`, so labels could be neither read nor written. The expiry now travels inside the payload as a marked envelope (`{"__pm":1,"value","expiresAt"}`); operator-entered secrets stay plain strings and the engine's read-creds secret is written bare. Still to confirm on the VM: `set` adds the daily version and old ones actually reach DESTROYED (retirement is best-effort). | `adapters/secret-manager.ts` | 🟠 Session shows disconnected / token write fails. |
| 4 | **Confirmed live 2026-09-25 for Dhan** (see §11.1 #18); Kite still to confirm. **Dhan consent flow is now server-side** (transcribed from DhanHQ v2 docs → Authentication → API Key & Secret): `POST /v1/auth/dhan/login-url` calls `POST https://auth.dhan.co/app/generate-consent?client_id=…` (headers `app_id`/`app_secret`) and returns `https://auth.dhan.co/login/consentApp-login?consentAppId=…`; Dhan's page redirects to the app's registered Redirect URL — `GET https://portfolio.swasthionline.com/v1/auth/dhan/redirect?tokenId=…` — which calls `GET https://auth.dhan.co/app/consumeApp-consent?tokenId=…`, checks `dhanClientId` against `dhan-client-id`, stores the token and 302s to `pm://broker-callback?broker=dhan&status=…`. Unverified live: the exact JSON field names (`consentAppId`, `accessToken`, `expiryTime` as IST wall-clock, `dhanClientId`), whether the redirect keeps the registered URL's path intact, and the 25-consents/day cap. `/v1/auth/dhan/callback` now always answers 400 NOT_SUPPORTED. **Kite** completes the same way on `/v1/auth/kite/redirect` (`request_token` + echoed `state`), see §11.2. | `services/session.ts`, `packages/broker-dhan/src/consent.ts` | 🟠 Daily Dhan login cannot complete until confirmed. |
...
| 15 | **Observed 2026-09-25:** `api.dhan.co` is CloudFront — successive lookups returned `108.158.46.x` then `18.161.246.x` — so the IP allowlist filled 10 minutes earlier matched the engine's own lookup only by luck; a miss is a dropped connection and a tick failing closed with `fetch failed`. `egress-allowlist.sh` now resolves via `host` (DNS only) and **pins every allowlisted hostname in `/etc/hosts`** to one address the sets contain, refreshed each cycle; network errors also carry their `cause` (ENETUNREACH/EAI_AGAIN…). Verify: `grep -A8 'BEGIN pm-egress-allowlist' /etc/hosts` on the VM, and no `pm-strategy-egress-drop` lines in `journalctl -k` during ticks. Two more findings from the drop log: (a) `meta skuid` never matches a socketless kernel packet (RST/ACK for stale or scanned connections), so the old `!= pm-strategy accept` rule sent those to the drop — ~190 mis-attributed "drops" per 10 min, none of them the engine's; the chain now matches pm-strategy positively and jumps. (b) A refresh replaced the sets while the engine's keep-alive connections were still open to the previous answers, so they died mid-request — `ct state established,related accept` now keeps an allowed connection usable; only OPENING one is controlled. Residual: a pinned CloudFront node retiring inside a 10-minute cycle. | `infra/vm/egress-allowlist.sh`, `nftables-strategy-egress.conf` | 🟠 Random tick failures — strategies silently miss evaluations. |
