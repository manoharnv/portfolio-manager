...
| 11 | First-boot-only clone/build in `bootstrap.sh`: after the first scheduled start, confirm `journalctl -u google-startup-scripts` shows "existing checkout … not touching it" and that boot-to-`/health` is ≤ 3 min. | `vm/bootstrap.sh` | 🟠 A slow or rebuilding boot could miss the 09:00 pre-open tick. |
...
| 7 | **Dhan daily login stops at VERIFY-LIVE**: Dhan's consent flow mints the access token outside the app, and forwarding a broker token through the app would violate docs/06 §6.7. Confirm the real consent/`request_token` mechanics and finish the exchange server-side. Kite is end-to-end. | `src/lib/brokerLogin.ts` | 🟠 No Dhan trading day can start from the app until resolved. |
